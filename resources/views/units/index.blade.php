...
<center>
    ...
    <div class="btn-group" role="group" aria-label="Basic example">
        <button class="btn btn-success" onclick="window.location.href='{{route('medidas.create')}}'"> Crear unidad de medida
        </button>
    </div>
</center>
...
<table class="table table-bordered table-hover" id="tablaObjeto">
    ...
    @foreach ($medidas as $key => $unidad)
    <tr>
        <td>
            {{ $unidad->name }}
        </td>
        <td>
            <form action="{{ route('medidas.show',$unidad->id) }}" method="get">
                @csrf
        ...
        <td>
            <center>
                <form action="{{ route('medidas.edit',$unidad->id) }}" method="get" style="display: inline-block">
                    @csrf
                ...
                <form action="{{ route('medidas.destroy',$unidad->id) }}" method="POST" style="display: inline-block;">
                    @csrf
                    @method('DELETE')
