...
<center>
<div class="btn-group">
    <button class="btn btn-success" onclick="window.location.href='{{ route('categorias.index') }}'">Regresar al menú</button>
</div>
</center>
...
@csrf
    ...
    <div class="form-group">
        ...
        <input required type="text" name="name" id="name" class="form-control" @if ($Accion !='Crear' )
            value="{{ $categoria->name }}" @if ($Accion=='Mostrar' ) disabled @endif @endif>
    </div>
    <div class="form-group">
        <label for="description">Descripción: </label>
        <textarea required class="form-control" name="description" id="description" rows="3" @if ($Accion=='Mostrar' )
            disabled @endif>@if ($Accion != 'Crear'){{ $categoria->description }}@endif</textarea>
    </div>
